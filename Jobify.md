...
- complete response (user:{email:user.email, lastName:user.lastName, location:user.location, name:user.name}, token, location:user.location)
...
```js appContext.js - deleteJob error
if(error.response.status = 401) return;
dispatch({
  type:DELETE_JOB_ERROR,
  payload:{msg:error.response.data.msg}
})
- after the catch block clearAlert()
```
...
```js JobsContainer.js
- showAlert from useAppContext
- import Alert
- after the Wrapper, if show alert is true, display alert
```
